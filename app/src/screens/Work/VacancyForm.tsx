...
const schema = yup.object({
  ...
  minSalary: yup.number().typeError('Это поле должно быть числом').required(locale.fields.required),
  maxSalary: yup.number().typeError('Это поле должно быть числом').required(locale.fields.required),
  workExperience: yup.number().typeError('Это поле должно быть числом').required(locale.fields.required),
});
...
const VacancyForm = ({ navigation }: Props) => {
  ...
  return (
    <KeyboardAvoidingView behavior={Platform.OS === 'ios' ? 'padding' : 'height'}>
      <ScrollView style={formStyles.wrapper} nestedScrollEnabled>
        <Formik
